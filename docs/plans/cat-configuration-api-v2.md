...
- [x] (2026-08-27 16:26Z) Centralized ordinal case-insensitive CAT client matching and changed runtime diagnostics to report the startup snapshot's configured counts and active ports.
- [x] (2026-08-27 16:37Z) Replaced Configuration API v1 mappings with v2 and added snapshot-derived configured/effective CAT response records, example configuration, and README guidance.
- [x] (2026-08-27 16:40Z) Added loopback startup and shared-profile coverage, ran the release suite, and manually verified the v2 HTTP response and active listener startup with environment-only configuration.
...
All milestones are complete. CAT configuration binds `Profiles` and `Clients`, rejects unknown keys such as the removed root `PortSettings`, and aggregates path-qualified validation failures. A singleton provider captures validated configuration once, returns defensive copies of configured data, and resolves active bindings in profile/port order without radio presence. `CatPortHostedService` is the only CAT hosted service; it creates children through a factory, rolls them back after partial startup failure, observes unexpected completion/faults, and stops all children on application shutdown. CAT client matching is ordinal case-insensitive in both slice ownership and client-handle lookup. Runtime logs report configured profile/client counts and only active listener ports, or `disabled`. Configuration routes are v2-only and the CAT settings endpoint returns dedicated configured/effective response records rather than mutable options. The README and example configuration document the breaking profile/client shape. Focused loopback tests prove an active binding opens TCP without creating an inactive listener, and shared-profile tests prove only the enabled client is resolved. The release suite passed 81 tests with one opt-in hardware test skipped. A manual process started from a temporary content root using environment-only configuration, logged active port `62001`, returned the expected v2 configured/effective response, and shut down cleanly. Full repository formatting verification still reports pre-existing findings outside this work; scoped formatting for changed files passed.
...
Plan created 2026-08-27 and updated 2026-08-27: all milestones are complete. The work delivered the breaking profile/client schema, startup validation, resolved binding provider, CAT lifecycle coordinator, case-insensitive client matching, startup diagnostics, v2 Configuration API, documentation, automated coverage, and manual runtime verification.
